...
def set_atom_subpockets(molecule, groups):
    if molecule.GetNumAtoms() != groups.shape[0]:
        print("Incorrect number of groups for molecule")
        return
...
def draw_molecules_with_subpockets(molecules, output_dir, output_prefix, palette=[(1,0.2,0.2), (1,0.55,0.15), (1,1,0.2), (0.2,1,0.2), (0.3,0.3,1), (0.5,1,1), (1,0.5,1)], panelx=250, panely=200, molsPerRow=4, rowsPerPage=6, name_parameter="_Name", scoring_parameter="r_i_glide_gscore"):
    """TODO: add choice of scoring parameter, choice of palette, filter compounds by subpocket occupancy
    ...
    # filenames = ['/home/rsmith/Dropbox (Schlessinger lab)/Schlessinger lab Team Folder/rsmith/papers/pyvol/sm/test_cmpd_table_{0}.png'.format(x) for x in range(len(page_data))]
...
def compound_occupancy(pose_file, pocket_file, output_dir, output_prefix=None, name_parameter="_Name", scoring_parameter="r_i_glide_gscore", pocket_tolerance=3, panelx=250, panely=200, molsPerRow=4, rowsPerPage=6):
...
if __name__ == "__main__":
    ...
    name_parameter = "_Name"
    scoring_parameter = "r_i_glide_gscore"
    pocket_tolerance=3
    ...
    #
    # prop_groups = subpocket_spheres.propagate_groups_to_external(pos)
    # set_atom_subpockets(m1, prop_groups)
    # draw_molecules_with_subpockets([m1], [(1,0,0), (0,1,0), (0,0,1)])

    for m in ms:
        set_atom_subpockets(m, subpocket_spheres.propagate_groups_to_external(m.GetConformers()[0].GetPositions(), tolerance=pocket_tolerance))
    draw_molecules_with_subpockets(ms)
